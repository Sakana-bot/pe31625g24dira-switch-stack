...
仓库根目录的 `VERSION` 是管理软件和 deploy-kit 的唯一版本来源。当前预发布版本为 `1.3.0-rc.2`，正式版本仍为 `1.2.0`。

| 组件 | 当前版本 | 规则 |
| --- | --- | --- |
| 管理软件与 deploy-kit | `1.3.0-rc.2` | 共用唯一版本；不兼容变化提升主版本，新功能提升次版本，兼容修复提升补丁版本 |
...
日常开发在周期开始时确定下一个版本并加 `-dev`，只提交到 `dev`。需要实机分发测试时可从 `dev` 创建 `-rc.N` 预发布；普通提交和小修复不会创建 Release。只有维护者明确要求正式发布后，才合并到 `main` 并创建不带后缀的正式版本。
