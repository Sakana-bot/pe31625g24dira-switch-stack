...
仓库根目录的 `VERSION` 是管理软件和 deploy-kit 的唯一版本来源。当前 `dev` 开发版本为 `1.2.0`；只有用户明确确认发布后，才会合并到 `main` 并创建同版本 Release。

| 组件 | 当前版本 | 规则 |
| --- | --- | --- |
| 管理软件与 deploy-kit | `1.2.0` | 共用唯一版本；不兼容变化提升主版本，新功能提升次版本，兼容修复提升补丁版本 |
