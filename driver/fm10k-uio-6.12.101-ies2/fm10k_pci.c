...
{
	struct fm10k_intfc *interface = fm10k_timer_container_of(interface, t,
							 service_timer);
...
void fm10k_qv_free_irq(struct fm10k_intfc *interface)
{
	...
	entry = &interface->msix_entries[NON_Q_VECTORS(&interface->hw) + vector];
...
int fm10k_qv_request_irq(struct fm10k_intfc *interface)
{
	...
	entry = &interface->msix_entries[NON_Q_VECTORS(&interface->hw)];
			 ...
			 const struct pci_device_id *ent)
{
	...
	/* BAR4 is the FM10840 switch register aperture exposed through UIO. */
...
static int fm10k_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (err)
		dev_warn(&pdev->dev, "Failed to start UIO interface: %d\n", err);
...
static void fm10k_remove(struct pci_dev *pdev)
{
	...
	fm10k_timer_delete_sync(&interface->service_timer);
