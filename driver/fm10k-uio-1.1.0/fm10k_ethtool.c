...
static int fm10k_set_priv_flags(struct net_device *netdev, u32 priv_flags)
{
	...
	} else if (test_and_clear_bit(FM10K_FLAG_IES_MODE, interface->flags)) {
		set_bit(FM10K_FLAG_RESET_REQUESTED, interface->flags);
	}
			       ...
			       struct ethtool_channels *ch)
{
	...
	ch->max_other = NON_Q_VECTORS(&interface->hw);
			      ...
			      struct ethtool_channels *ch)
{
	...
	if (ch->other_count != NON_Q_VECTORS(&interface->hw))
		return -EINVAL;
